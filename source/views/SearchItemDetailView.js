...
export default function SearchItemDetailView({ ...props }) {
    ...
    const [label, setLabel] = useState(all_constants.search.search_item_detail_view.button.add_to_basket_default)
    ...
    const onChangeLabelAndCounter = (counter) => {
        ...
        label_start = all_constants.search.search_item_detail_view.button.add_to_basket_start
        label_end = all_constants.search.search_item_detail_view.button.add_to_basket_end
    ...
    return (
        <View style={{ flex: 1 }}>
            ...
            <View style={{ flex: 1 }}>
                <View style={{ flex: 1.3, backgroundColor: 'white', paddingLeft: '4%', paddingRight: '4%' }}>
                    <View style={{ flex: 1.2 }}>
                        <Text numberOfLines={1} style={{ fontSize: 26, fontWeight: 'bold' }}>
                            {props.route.params.item.dish_name}{' '}
                            <CountryFlag isoCode={props.route.params.item.dish_country_code} size={22} />
                        </Text>
                    </View>

                    <View style={{ flex: 1.2 }}>
                        <Text style={{ fontSize: 22 }}>
                            {props.route.params.item.dish_price}{all_constants.currency_symbol}
                        </Text>
                    </View>
                ...
                <View style={{ flex: 0.7, backgroundColor: 'white', paddingLeft: '4%', paddingRight: '4%' }}>
                    <Text numberOfLines={1} style={{ fontSize: 20, fontWeight: 'bold' }}>
                        {all_constants.search.search_item_detail_view.delivery.delivery_info}
                    </Text>
                ...
                <View style={{ flex: 0.5, justifyContent: 'center', alignItems: 'center', bottom: '2%' }}>
                    <CustomButton
                        label={label}
                        height={50}
                        border_width={3}
